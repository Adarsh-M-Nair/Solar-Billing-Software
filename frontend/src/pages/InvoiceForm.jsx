  ...
  return (
    <div className="space-y-6 animate-in fade-in max-w-4xl mx-auto">
      <div className="flex items-center gap-4">
        <button onClick={() => navigate('/invoices')} className="p-2 rounded-full hover:bg-slate-200 transition-colors">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="mt-6 flex justify-end">
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 min-w-[250px]">
              ...
              <p className="text-xs text-slate-400 border-t border-slate-200 pt-2 text-center">
                GST is automatically calculated upon submission based on the customer's state.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-end gap-4">
           ...
           <button type="submit" disabled={isSubmitting} className="btn-primary flex items-center gap-2">
             <Receipt className="w-4 h-4"/> {isSubmitting ? 'Saving...' : 'Generate Invoice'}
           </button>
        </div>
      </form>
    </div>
